class Map {
  ...
  void insert(Boolean edge, PVector point){
    if (edge){
      this.inside_edge.add(0, point);
    } else {
      this.outside_edge.add(0, point);
    }
  }
  
  void undo(Boolean edge) {
    if (edge) {
      if (this.inside_edge.size() > 0) this.inside_edge.remove(0);
    } else {
      if (this.outside_edge.size() > 0) this.outside_edge.remove(0);
    }
  }
